...
namespace Biblioteca.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserEmailStore<BibliotecaUser> _emailStore;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //await _firstNameStore.SetUserNameAsync(user, Input.FirstName, CancellationToken.None);
                //await _lastNameStore.SetUserNameAsync(user, Input.LastName, CancellationToken.None);
        ...
        private BibliotecaUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BibliotecaUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BibliotecaUser)}'. " +
                    $"Ensure that '{nameof(BibliotecaUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BibliotecaUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BibliotecaUser>)_userStore;
        }
    }
}
